...
export default function GeofencingManagementSystem({
  ...
  const [drawMode, setDrawMode] = useState<"Polygon" | "Circle" | "Rectangle" | "None">("None");
  ...
  const activeArea = areas.find(a => a.id === selectedAreaId) || null;
  ...
  const filteredAreas = useMemo(() => {
    return areas.filter(area => {
      ...
      const matchManager = filterManager === "All" || area.assignedManagerId === filterManager;
  ...
  const statistics = useMemo(() => {
    ...
    const managersCount = MOCK_MANAGERS.length;
    ...
    const activeCoverageSqKm = areas.filter(a => a.status === "Active").reduce((acc, a) => acc + a.coverageSqKm, 0);
    // Supposing Kolkata metropolitan area evaluated around 205 sq.km
    const coveragePercent = Math.min(100, Math.round((activeCoverageSqKm / 180) * 100));
  ...
  const handleAddNewAreaClick = () => {
    ...
    triggerToast("Drawing Mode Deployed", "Form parameters opened. Click nodes directly on the OpenStreetMap overlay to draw the geofence perimeter.", "info");
  };
  ...
  const handleDuplicateArea = (area: AreaZoneConfig) => {
    ...
    triggerToast("Service Area Duplicated", `Successfully configured draft clone of "${area.name}".`, "success");
  };
  ...
  const handleDeleteArea = (id: string) => {
    ...
    triggerToast("Service Area Terminated", `Successfully scrubbed "${target.name}" from active geofencing lists.`, "error");
  };

  const handleToggleState = (area: AreaZoneConfig) => {
    const targetStatus = area.status === "Active" ? "Inactive" : "Active";
    setAreas(prev => prev.map(a => a.id === area.id ? { ...a, status: targetStatus } : a));
    triggerToast("Operation Status Updated", `"${area.name}" marked as ${targetStatus}.`, "success");
  };

  // Click on "Apply Boundary" in map workspace saves draft points to form state
  const handleSavePointsFromMap = () => {
    if (draftPoints.length === 0) {
      triggerToast("Empty boundary", "Draw some coordinates before saving.", "error");
      return;
    }
    setAreaForm(prev => ({
      ...prev,
    ...
    triggerToast("Path boundaries Locked", `${draftPoints.length} map path coordinate nodes secured to area draft.`, "success");
  };
  ...
  const handleConfigFormSubmit = (e: React.FormEvent) => {
    ...
    if ((areaForm.points || []).length === 0) {
      triggerToast("Missing Boundary Layer", "You must click and outline a geofence footprint on the map workspace before submitting.", "error");
      return;
    }
    ...
    if (!areaForm.assignedManagerId) {
      warnings.push("Unassigned Local Operations Manager. Every delivery sector requires an administrative lead.");
    }
    // Check for unassigned Hub
    if (!areaForm.assignedHubId) {
      warnings.push("Hub Allocation Missing. Delivery dispatch cannot function without an assigned logistics node direction.");
    }
    // Check for primary Pin validation
    if (!areaForm.primaryPinCode || !/^\d{6}$/.test(areaForm.primaryPinCode)) {
      warnings.push("Malformed or Missing Main PIN Code. Operational routing requires an active postal identifier.");
    }
    // Check overlapping pin codes in existing list
    const existsPin = areas.some(a => a.id !== areaForm.id && a.primaryPinCode === areaForm.primaryPinCode);
    if (existsPin) {
      warnings.push(`Overlapping Geofence warning: Primary Pin Code ${areaForm.primaryPinCode} is already claimed by another active area.`);
    }
  ...
  const handleConfirmSaveFinal = (forceAsDraft: boolean = false) => {
    const finalZone: AreaZoneConfig = {
      ...
      createdDate: areaForm.createdDate || new Date().toISOString().split("T")[0]
    };

    if (isEditing) {
      setAreas(prev => prev.map(a => a.id === finalZone.id ? finalZone : a));
      triggerToast("Area Successfully Updated", `Geofence coordinates and delivery rates saved for "${finalZone.name}".`, "success");
    } else {
      setAreas(prev => [...prev, finalZone]);
      setSelectedAreaId(finalZone.id);
      triggerToast("New Delivery Area Published", `"${finalZone.name}" has been registered into the live delivery network.`, "success");
    }
  ...
  const handleExportAreas = () => {
    ...
    downloadAnchor.setAttribute("download", `FeastFlow_Service_Areas_${Date.now()}.json`);
    document.body.appendChild(downloadAnchor);
    downloadAnchor.click();
    downloadAnchor.remove();
    triggerToast("Data Export Completed", "Successfully packed and compiled delivery territory configurations to JSON.", "success");
  };

  const handleImportAreas = () => {
    triggerToast("Bulk Area Importer", "Upload configuration database via the workspace settings panel.", "info");
  };

  return (
    <div id="area-management-root" className="min-h-screen bg-slate-900 text-zinc-100 font-sans antialiased p-6 transition-colors duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800 pb-6 mb-6">
        <div>
          <span className="text-[11px] font-black uppercase text-rose-500 tracking-widest block mb-1">Logistics & Operations Control</span>
          <h1 className="text-3xl font-black text-white tracking-tight flex items-center gap-2">
            Area Management & Geofencing
          </h1>
          <p className="text-xs text-gray-400 font-medium max-w-2xl leading-relaxed mt-1">
            Design, regulate, and scale regional dispatch boundaries utilizing interactive OpenStreetMap (OSM) tools, 
            multivariable ZIP mappings, staff manager delegation, and real-world DoorDash-inspired surge parameters.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          ...
          <button
            onClick={() => {
              ...
              triggerToast("Toggling Performance Insights", "Interactive operational overview charting rendered.", "info");
            }}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-4 mb-6">
        ...
        <div className="bg-slate-900 p-4 rounded-2xl border border-slate-900 space-y-1 shadow-md">
          ...
          <div className="text-2xl font-black text-white">{statistics.active}</div>
          <div className="text-[10px] text-gray-400 font-semibold flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-ping inline-block" /> Live dispatching
          </div>
        </div>
        ...
        <div className="bg-slate-900 p-4 rounded-2xl border border-slate-900 space-y-1 shadow-md">
          ...
          <div className="text-2xl font-black text-white">{statistics.managers}</div>
          <div className="text-[10px] text-gray-400 font-semibold italic">Delegated field leads</div>
        </div>
        ...
        <div className="bg-slate-900 p-4 rounded-2xl border border-slate-900 space-y-1 shadow-md">
          ...
          <div className="text-2xl font-black text-white">{statistics.hubs}</div>
          <div className="text-[10px] text-gray-400 font-semibold italic">Central warehouses</div>
        </div>
        ...
        <div className="bg-slate-900 p-4 rounded-2xl border border-slate-900 space-y-1 shadow-md">
          ...
          <div className="text-2xl font-black text-white">{statistics.linkedPins}</div>
          <div className="text-[10px] text-gray-400 font-semibold italic">Active PIN codes mapped</div>
        </div>
        ...
        <div className="bg-slate-900 p-4 rounded-2xl border border-slate-900 space-y-1 shadow-md">
          ...
          <div className="text-2xl font-black text-white">{statistics.coveragePercent}%</div>
          <div className="text-[10px] text-gray-400 font-semibold italic">Kolkata core region coverage</div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-2.5 items-center flex-1">
          ...
          <div className="flex flex-wrap gap-2 text-xs font-semibold">
            ...
            >
              ...
              {MOCK_MANAGERS.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
            ...
            >
              ...
              {MOCK_HUBS.map(h => <option key={h.id} value={h.id}>{h.name}</option>)}
            </select>
          </div>
        </div>

        {/* Clear filters trigger */}
        {(searchQuery || filterCity !== "All" || filterStatus !== "All" || filterManager !== "All" || filterHub !== "All") && (
          <button
            onClick={() => {
              setSearchQuery("");
              setFilterCity("All");
              ...
              setFilterHub("All");
              triggerToast("Filters Cleared", "Displaying all registered service sectors.", "info");
            }}
      ...
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-sm font-black text-white uppercase tracking-wider flex items-center gap-1.5">
          <Activity className="w-4 h-4 text-rose-500" /> Interactive Platform Workspace
        </h3>
      ...
      {viewMode === "split" && (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
          ...
          {/* Map canvas screen area (7 cols span) */}
          <div className="xl:col-span-8 space-y-4">
            ...
            {/* Dynamic Rider and Restaurant mapping breakdown details under selected zone */}
            {activeArea && (
              <div className="bg-slate-900 p-5 rounded-3xl border border-slate-900 space-y-4 shadow-xl">
                <div className="flex justify-between items-center border-b border-slate-900 pb-3">
                  <div>
                    <h4 className="text-sm font-black text-white flex items-center gap-1.5">
                      <ShoppingBag className="w-4 h-4 text-emerald-400" /> Merchant & Courier Containment mapping
                    </h4>
                    <p className="text-[10px] text-gray-400 font-semibold leading-relaxed">
                      Sectors are automatically checked for active restaurants and riders registered inside coordinate polygon borders.
                    </p>
                  </div>
                  <span className="text-[10px] font-black text-gray-300 bg-slate-900 border border-slate-800 px-3 py-1 rounded-xl">
                    Center Centroid: {activeArea.points[0] ? `${activeArea.points[0].x}%, ${activeArea.points[0].y}%` : "Undrawn"}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-[11px]">
                  ...
                  <div className="space-y-2">
                    <span className="text-gray-400 uppercase font-black text-[9px] tracking-wider block">Restaurants active ({activeArea.restaurantsCount})</span>
                    <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                      {restaurants.slice(0, 3).map((rest, index) => (
                        <div key={rest.id} className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-820 flex justify-between items-center">
                          <div>
                            <span className="text-white font-extrabold block">{rest.name}</span>
                            <span className="text-[9px] text-[#22C55E] font-black flex items-center gap-1">
                              ● On-Duty Service · Rating {rest.rating}★
                            </span>
                          </div>
                          <button
                            onClick={() => triggerToast("Action Handled", `${rest.name} reallocated to new boundary layout.`, "success")}
                            className="bg-slate-800 hover:bg-slate-755 text-zinc-100 p-1 px-2 text-[9px] font-black rounded-lg uppercase cursor-pointer"
                          >
                            Re-Route
                          </button>
                        </div>
                  ...
                  {/* Riders contained */}
                  <div className="space-y-2">
                    <span className="text-gray-400 uppercase font-black text-[9px] tracking-wider block">Couriers on-duty ({activeArea.ridersCount})</span>
                    <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                      {riders.slice(0, 3).map((r, index) => (
                        <div key={r.id} className="p-2.5 rounded-xl bg-slate-900/60 border border-slate-820 flex justify-between items-center">
                          <div>
                            <span className="text-white font-extrabold block">{r.name}</span>
                            <span className="text-[9px] text-[#38BDF8] font-bold block">{r.vehicleNumber} ({r.status})</span>
                          </div>
                          <div className="flex gap-1">
                            <button
                              onClick={() => triggerToast("Rider Shift Complete", `Transferred active shift of ${r.name}.`, "info")}
                              className="bg-slate-900 hover:bg-slate-800 text-slate-300 p-1 px-2 text-[9px] font-black rounded-lg uppercase cursor-pointer"
                            >
                              Transfer
                            </button>
                          </div>
          ...
          {/* Configuration Settings Slider / Component (4 cols span) */}
          <div className="xl:col-span-4 bg-slate-900 p-5 rounded-3xl border border-slate-900 max-h-[850px] overflow-y-auto shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-900 pb-3">
              <div>
                <h3 className="text-sm font-black text-white">
                  {showAddPanel ? `${isEditing ? "Edit" : "Draw"} Service Zone` : "Zone Specifications"}
                </h3>
                <p className="text-[10px] text-gray-500 font-semibold italic">Configure geofence borders, mapping codes and logistics leads.</p>
              </div>
              {showAddPanel && (
                <button
                  onClick={() => {
            ...
            {showAddPanel ? (
              ...
              <div className="space-y-5">
                {activeArea ? (
                  <div className="space-y-4 text-xs font-semibold text-gray-300">
                    ...
                    <div className="p-4 bg-slate-900/40 rounded-2xl border border-slate-800 space-y-3">
                      <div className="flex justify-between items-start">
                        <div>
                          <h4 className="text-sm font-black text-white">{activeArea.name}</h4>
                          <span className="text-[10px] text-gray-400 font-mono font-bold block">{activeArea.code}</span>
                        </div>
                        <span className={`p-1 px-2 text-[9px] font-black uppercase rounded ${activeArea.status === "Active" ? "bg-emerald-500/10 text-emerald-400" : activeArea.status === "Inactive" ? "bg-rose-500/10 text-rose-500" : "bg-amber-500/10 text-amber-500"}`}>
                          {activeArea.status}
                        </span>
                      </div>
                      
                      <p className="text-[10px] leading-relaxed font-medium italic text-gray-400">{activeArea.description}</p>
                      
                      <div className="h-[1px] bg-slate-900" />

                      <div className="grid grid-cols-2 gap-3 text-center">
                        <div className="p-2 bg-slate-900 rounded-xl">
                          <span className="text-[9px] text-gray-400 block">Radius Area</span>
                          <span className="text-white font-mono font-black">{activeArea.coverageSqKm} sq km</span>
                        </div>
                        <div className="p-2 bg-slate-900 rounded-xl">
                          <span className="text-[9px] text-gray-400 block">Local Population</span>
                          <span className="text-white font-mono font-black">~{(activeArea.populationEstimate / 1000).toFixed(0)}k</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-slate-900/40 rounded-2xl border border-slate-800 space-y-2">
                      <span className="text-gray-400 font-black uppercase text-[9px] tracking-wider block">Assigned Dispatch Logistics Hub</span>
                      {MOCK_HUBS.find(h => h.id === activeArea.assignedHubId) ? (
                        (() => {
                          const hub = MOCK_HUBS.find(h => h.id === activeArea.assignedHubId)!;
                          return (
                            <div>
                              <div className="text-white font-extrabold text-[11px] mb-0.5">{hub.name}</div>
                              <div className="text-[10px] text-gray-400">Lead Manager: <span className="text-white font-bold">{hub.manager}</span></div>
                              <div className="text-[10px] text-gray-500 font-mono mt-0.5">{hub.address}</div>
                            </div>
                          );
                        })()
                      ) : (
                        <div className="text-rose-500 italic font-bold">No Hub linked yet. Set link via edit.</div>
                      )}
                    </div>

                    {/* Mapped Supervisor */}
                    <div className="p-4 bg-slate-900/40 rounded-2xl border border-slate-800 space-y-2">
                      <span className="text-gray-400 font-black uppercase text-[9px] tracking-wider block">Local Operations Lead</span>
                      {MOCK_MANAGERS.find(m => m.id === activeArea.assignedManagerId) ? (
                        (() => {
                          const mgr = MOCK_MANAGERS.find(m => m.id === activeArea.assignedManagerId)!;
                          return (
                            <div className="flex justify-between items-center bg-slate-900 p-2.5 rounded-xl border border-slate-900">
                              <div>
                                <span className="text-white font-extrabold block">{mgr.name}</span>
                                <span className="text-[9px] text-gray-400">{mgr.email}</span>
                              </div>
                              <span className="px-2 py-0.5 rounded bg-rose-600/10 text-rose-400 text-[8px] uppercase">{mgr.role}</span>
                            </div>
                          );
                        })()
                      ) : (
                        <div className="text-rose-500 italic font-bold">Unassigned field lead. Allocate immediately.</div>
                      )}
                    </div>

                    {/* Pricing Configs summary */}
                    <div className="p-4 bg-slate-900/40 rounded-2xl border border-slate-800 space-y-3">
                      <span className="text-gray-400 font-black uppercase text-[9px] tracking-wider block">Pricing Coefficient Thresholds</span>
                      <div className="grid grid-cols-2 gap-2 text-[10px]">
                        <div>Base Fee: <span className="text-white font-black">₹{activeArea.baseDeliveryFee}</span></div>
                        <div>Per KM Charge: <span className="text-white font-black">₹{activeArea.perKmCharge}</span></div>
                        <div>Max Radius limit: <span className="text-white font-black">{activeArea.maxDeliveryRadius} km</span></div>
                        <div>Free Delivery threshold: <span className="text-white font-black">₹{activeArea.freeDeliveryThreshold}</span></div>
                      </div>
                      <div className="p-2 bg-rose-900/20 text-rose-300 border border-rose-900/30 rounded-xl text-[10px] flex items-center gap-2">
                        ...
                        <span>Surge Pricing: <span className="font-bold">{activeArea.surgePricingEnabled ? `Active (${activeArea.peakHourMultiplier || "1.2"}x multiplier)` : "Disabled"}</span></span>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 justify-end pt-2">
                      <button
                        ...
                        className={`p-2 px-3 hover:bg-slate-800 text-slate-300 border font-extrabold rounded-lg text-[10px] uppercase cursor-pointer ${activeArea.status === "Active" ? "border-emerald-500/20 text-emerald-400" : "border-slate-800"}`}
                      >
                        {activeArea.status === "Active" ? "Deactivate Zone" : "Activate Zone"}
                      </button>
                      <button
                  ...
                  <div className="p-10 text-center select-none space-y-2">
                    <span className="text-gray-500 block">No Active Delivery Area Selected</span>
                    <button
                      onClick={handleAddNewAreaClick}
      ...
      {viewMode === "table" && (
        <div className="bg-slate-900 rounded-3xl border border-slate-900 overflow-hidden shadow-2xl select-none">
          <div className="p-5 border-b border-slate-900 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-black text-white">Service Area Register Database</h3>
              <span className="text-[10px] text-gray-500 font-semibold italic">Directory of registered municipal sectors, coordinate bounds, and supervisor codes.</span>
            </div>
            ...
            <table className="w-full text-left text-xs text-gray-300 border-collapse">
              ...
              <tbody className="divide-y divide-slate-850">
                {filteredAreas.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="p-10 text-center text-gray-500 italic font-semibold">No operational sectors matched the search query filter guidelines.</td>
                  </tr>
                ) : (
                  filteredAreas.map((area, index) => {
                    const hubObj = MOCK_HUBS.find(h => h.id === area.assignedHubId);
                    const managerObj = MOCK_MANAGERS.find(m => m.id === area.assignedManagerId);

                    return (
                      <tr key={area.id} className="hover:bg-slate-900/40 transition-all font-semibold">
                        {/* Name & Code */}
                        <td className="p-4 pl-6 space-y-0.5">
                          <span className="text-white block font-black text-xs">{area.name}</span>
                          <span className="text-[9px] text-gray-400 font-mono block">{area.code} · {area.city}</span>
                        </td>
                        {/* ZIP code */}
                        ...
                        <td className="p-4 text-gray-200 text-[11px]">
                          {managerObj ? managerObj.name : <span className="text-rose-455 text-[10px] font-black italic">Unassigned</span>}
                        </td>
                        {/* Hub */}
                        <td className="p-4 text-gray-200 text-[11px]">
                          {hubObj ? hubObj.name : <span className="text-rose-455 text-[10px] font-black italic">Unassigned</span>}
                        </td>
                        ...
                        {/* Status badge */}
                        <td className="p-4">
                          <span className={`p-1 px-2 text-[9px] font-black uppercase rounded ${area.status === "Active" ? "bg-emerald-500/10 text-emerald-400" : area.status === "Inactive" ? "bg-rose-500/10 text-rose-500" : "bg-amber-500/10 text-amber-500"}`}>
                            {area.status}
                          </span>
                        </td>
                        {/* Actions group */}
        ...
        <div className="bg-slate-900 p-6 rounded-3xl border border-slate-900 space-y-6 shadow-2xl animate-fade-in select-none">
          <div className="border-b border-slate-900 pb-4">
            <h4 className="text-base font-black text-white flex items-center gap-1.5">
              <BarChart2 className="w-5 h-5 text-amber-500" /> Operational Insights & Analytics
            </h4>
            <p className="text-[11px] text-gray-500 font-semibold">Track order throughput, local revenue indices, active rider clusters, and regional SLA success rates across sectors.</p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-820 space-y-3">
              <span className="text-gray-400 uppercase font-black text-[10px] block">Order Volume Throughput (Active Runs)</span>
              <div className="h-64" style={{ minHeight: "256px", minWidth: 0 }}>
                <SafeResponsiveContainer minHeight={256} minWidth={0}>
                  <BarChart data={MOCK_TRENDS}>
                    ...
                    <XAxis dataKey="areaName" stroke="#9ca3af" fontSize={10} tickLine={false} />
                    <YAxis stroke="#9ca3af" fontSize={10} tickLine={false} />
                    <Tooltip contentStyle={{ backgroundColor: "#020617", borderColor: "#1f2937", color: "#fff" }} />
                    <Bar dataKey="orders" fill="#e11d48">
                      {MOCK_TRENDS.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={index === 0 ? "#6366f1" : index === 1 ? "#3b82f6" : "#10b981"} />
                      ))}
                    </Bar>
                  </BarChart>
            ...
            <div className="bg-slate-900/60 p-4 rounded-2xl border border-slate-820 space-y-3">
              <span className="text-gray-400 uppercase font-black text-[10px] block">Gross Revenue Generation (Zonal Index)</span>
              <div className="h-64" style={{ minHeight: "256px", minWidth: 0 }}>
                <SafeResponsiveContainer minHeight={256} minWidth={0}>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 pt-3 text-[11px]">
            <div className="p-3.5 bg-slate-900 rounded-xl border border-slate-900">
              <span className="text-gray-500 block text-[9px] font-black uppercase">Average Delivery Success</span>
              <span className="text-emerald-500 text-base font-black font-mono">97.8%</span>
            </div>
            <div className="p-3.5 bg-slate-900 rounded-xl border border-slate-900">
              <span className="text-gray-500 block text-[9px] font-black uppercase">Average Dispatch Times</span>
              <span className="text-white text-base font-black font-mono">34.4 mins</span>
            </div>
            <div className="p-3.5 bg-slate-900 rounded-xl border border-slate-900">
              <span className="text-gray-500 block text-[9px] font-black uppercase">Cancellation Index</span>
              <span className="text-rose-500 text-base font-black font-mono">1.82%</span>
            </div>
            <div className="p-3.5 bg-slate-900 rounded-xl border border-slate-900">
              <span className="text-gray-500 block text-[9px] font-black uppercase">Active Restaurant Partners</span>
              <span className="text-blue-400 text-base font-black font-mono">183 venues</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 space-y-6 shadow-2xl animate-scale-up text-xs font-semibold">
            ...
            <div className="flex items-start gap-3.5">
              ...
              <div>
                <h4 className="text-base font-black text-white">Zone Publishing Security Handshake</h4>
                <p className="text-[11px] text-gray-500">Checking territory guidelines and potential boundary conflicts prior to node registration.</p>
              </div>
            </div>

            {/* Validation warnings pipeline output */}
            <div className="space-y-2">
              <span className="text-[10px] text-gray-400 uppercase font-black block tracking-wider">Validation Handshake Report</span>
              {validationWarnings.length === 0 ? (
                <div className="p-3 bg-emerald-500/10 text-emerald-400 rounded-xl border border-emerald-500/20 text-[11px] font-extrabold flex items-center gap-2">
                  <Check className="w-4 h-4" /> Perfect alignment. Boundary coordinates are secure and clear of overlap conflicts.
                </div>
              ) : (
                <div className="space-y-1.5">
                  {validationWarnings.map((warn, i) => (
                    <div key={i} className="p-3 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded-xl text-[10px] font-bold flex items-start gap-1.5">
                      <AlertTriangle className="w-4 h-4 text-amber-500 flex-shrink-0 mt-0.5" />
                      <span>{warn}</span>
                    </div>
                  ))}
                  <p className="text-[10px] text-gray-400 italic">You may bypass these warnings to register the layout as draft or override conflicts during startup phase.</p>
                </div>
              )}
            </div>
            ...
            <div className="p-4 bg-slate-900 rounded-2xl border border-slate-900 space-y-2.5 text-[10px] font-mono">
              <span className="text-gray-400 font-sans font-black uppercase text-[9px] tracking-wider block">Zone Configuration Summary</span>
              <div>Area Name: <span className="text-white font-extrabold font-sans">{areaForm.name}</span></div>
              <div>Primary ZIP code: <span className="text-[#E11D48] font-black">{areaForm.primaryPinCode}</span></div>
              <div>Estimated Coverage: <span className="text-white font-black">{areaForm.coverageSqKm} sq. km</span></div>
              <div>Linked Supervisor ID: <span className="text-white">{areaForm.assignedManagerId || "Unassigned"}</span></div>
              <div>Linked Delivery Hub ID: <span className="text-white">{areaForm.assignedHubId || "Unassigned"}</span></div>
              <div>Base Delivery Fee: <span className="text-white text-xs font-black">₹{areaForm.baseDeliveryFee}</span></div>
            </div>
